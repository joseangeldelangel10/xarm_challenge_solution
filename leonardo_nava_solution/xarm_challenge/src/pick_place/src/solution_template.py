...
class myNode():
  ...
  def tf_goal(self, goal):
    ...
    # ====== WE ROTATE 180 deg IN X THE REFERENCE FRAME OF THE GIVEN goal
    # ====== SO THAT THE GRIPPER REACHES IT WITH WRIST UP
    goal_orientation = [transformStamped.transform.rotation.x,
                        transformStamped.transform.rotation.y,
                        transformStamped.transform.rotation.z,
                        transformStamped.transform.rotation.w]

    x_rot = quaternion_from_euler(pi,0,0)
    print("=========== x_rot is: =============")
    print(x_rot)
    goal_orientation = quaternion_multiply(goal_orientation, x_rot)
    
    pose.orientation.x = goal_orientation[0]
    pose.orientation.y = goal_orientation[1]
    pose.orientation.z = goal_orientation[2]
    pose.orientation.w = goal_orientation[3]
    return pose
  ...
  def main(self):
    ...
    for i in range(3):
      ...
      aboveGoal = self.tf_goal(pick)

      aboveGoal.position.z += 0.1

      self.planner.goToPose(aboveGoal)      
