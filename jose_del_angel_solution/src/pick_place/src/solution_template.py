...
class Planner():
  ...
  def addObstacles(self):
    ...
    deposit_pose.header.frame_id = "world"
    box_pose.pose.position.x = 0
    box_pose.pose.position.y = 0
    box_pose.pose.position.z = -0.06
    box_pose.pose.orientation.w = 1.0
    box_name = "table"
    self.scene.add_box(box_name, box_pose, size=(2, 2, 0.04))
    return True
...
class myNode():
  def __init__(self):
    ...
    RedBoxPose = Pose()
    RedBoxPose.position.x = 0.20078 -0.015
    RedBoxPose.position.y = 0.51023 + 0.01
    RedBoxPose.position.z = -0.0003553 + 0.01
    RedBoxPose.orientation.x = 1.0
    RedBoxPose.orientation.w = 0.0

    BlueBoxPose = Pose()
    BlueBoxPose.position.x = 0.074797 
    BlueBoxPose.position.y = 0.55853 + 0.005
    ...
    GreenBoxPose.position.y = 0.19451 - 0.01
  ...
  def main(self):
    ...
    rospy.wait_for_service('Hola_crayola')
    
    for i in range(3):
      pick = self.getGoal("pick")
      
      self.planner.goToPose(self.targets[pick])
